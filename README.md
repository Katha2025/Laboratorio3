...
Ya que este valor depende tanto del pulso fotopletismográfico (PPGA) como el intervalo entre latidos (HBI). Fisiológicamente, una activación simpática puede producir vasoconstricción periférica, reduciendo la amplitud de la señal PPG. Esto es precisamente lo que logra el estímulo frío. El aumento de SPI observado durante el estímulo es compatible con una respuesta autonómica periférica ante un estímulo térmico. Sin embargo, es necesario resaltar que los resultados no determinan si el sujeto experimentó una determinada intensidad de dolor. 

Por otro lado, la disminución durante la recuperación demuestra que, una vez se retira el estimulo, la respuesta tendió a regresar a los valores en condición basal. A pesar de esto, los valores no son iguales a los basales, lo cuál se le puede atribuir a factores como que la respuesta autonómica no desaparece instantaneamente, una variación fisiológica normal o factores de la adquisición de la señal. En cuanto a la frecuencia cardiaca, la diferencia entre reposo y estímulo fue muy mínima, con tan solo 0.1 lpm de diferencia. En contraste, el SPI aumentó por casi 24 unidades. Con esto se puede analizar que el cambio del SPI no estuvo acompañado por un cambio importante en la frecuencia cardíaca promedio. El SPI va a depender de la amplitud de la señal PPG y el intervalo entre latidos, pero se observó como en este caso una modificación importante del SPI puede producirse principalmente por cambios en la señal pletismográfica y no necesariamente por una gran modificación de la frecuencia cardíaca promedio.

Continuando, en la figura 11 se visualiza una regularidad en la señal PPG antes del estímulo. En esta etapa, se produce la señal de referencia basal del experimento, y con ella se identifica la forma consistente de cada ciclo cardiáco. En la figura 12, se evidencia una modificación de las características fisiológicas extraídas de la señal PPG. La señal pierde amplitud, asociado a cambios en la perfusión periférica y en la respuesta autonómica inducida por el frío. Con la figura 13, la señal vuelve a aumentar su amplitud para eventualmente regresar a sus valores basales. Al comparar las figuras 11, 12 y 13 se permite observar que la modificación de la amplitud de la PPG coincide temporalmente con las diferentes etapas experimentales.

Para la detección de los eventos de la señal se utilizó el método del alpinista. Este algoritmo identifica los picos y los valles mediante el análisis de la tendencia ascendente y descendente de la señal. El método utiliza inicialmente un umbral de seis muestras consecutivas y posteriormente adapta dicho umbral de acuerdo con el número de muestras que mantienen una tendencia ascendente. Esta característica permite que el procedimiento se adapte a variaciones en la forma y duración de los pulsos PPG. Cada pico sistólico es una pulsación y cada valle permite determinar la amplitud del pulso. La diferencia entre pico y valle proporciona el PPGA y la distancia temporal emtre dos picos obtiene el HBI. Es popr esto que este método resulto indispensable para verificar si el sistema de captación de señal es correcto y para visualizar los cambios del valor de SPI. Una ventaja de este método es su carácter adaptativo, ya que no depende exclusivamente de un umbral de amplitud fijo. Esto es importante para una correcta identificaciónde picos y valles, ya que cualquier error en el cálculo del PPGA o HBI afecta el cálculo del SPI, de esta manera se asegura que las variaciones observadas correspondan a cambios reales y no a errores de procesamiento.

A pesar de esto, la modificación real de la señal no es la única cosa que puede producir una detección incorrecta. Hay factores como el ruido, movimiento, presión del dedo sobre el sensor o artefactos que pueden generar cambios en la pendiente que el algorítmo puede interpretar como eventos fisiológicos de la señal. Por ejemplo, el valor puntual de SPI igual a 100 observado como el máximo durante el estímulo frío es muy alto y puede no considerarse por sí solo como representativo de toda la respuesta fisiológica, sino como un valor causado por movimiento o presión del sensor durante la extracción de la señal PGG.

A partir de estos resultados es posible decir que el el sistema fue capaz de adquirir la señal PPG de manera continua, identificar sus principales eventos mediante el método del alpinista y calcular un índice SPI que presentó una variación temporalmente relacionada con la aplicación y retiro del estímulo frío.
